...
 
 ...
           {
               // Handle any errors securely by logging the detailed error message and displaying a generic message to the UI
               NinjaTrader.Code.Output.Process(string.Format("Error on requesting bars: {0}, {1}", errorCode, errorMessage), PrintTo.OutputTab1);
               outputBox.Text = "An error occurred while requesting bars. Please check the Output tab for details.";
               return;
           }
       }));
   }));
